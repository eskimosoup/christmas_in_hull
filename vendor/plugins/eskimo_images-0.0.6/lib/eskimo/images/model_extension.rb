  ...
  module ClassMethods
    ...
    def regenerate_samples!
      ...
      for instance in self.all
        puts "processing #{self.name} #{instance.id}"
        for image_type in self.images.keys
          puts "  #{image_type}"
          original_path = instance.send(image_type).path
          if original_path
            puts "  loading image at #{original_path}"
            image = Magick::Image::read(original_path).first
            puts "  cropping file"
            sample = image.resize_to_fill(100,100)
            sample_file = instance.send(image_type).path(:sample) 
            puts "  saving as #{sample_file}"
            FileUtils.mkdir_p(sample_file)
            FileUtils.rm_rf(sample_file)
